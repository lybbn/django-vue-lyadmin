<template>
    <div>
        <el-table size="small" height="calc(100vh - 160px)" border :data="tableData" v-loading="loadingPage" style="width: 100%">
            ...
            <el-table-column min-width="120" prop="image" label="封面图(317*189)">
            ...
            <el-table-column label="操作" fixed="right" width="180">
                ...
                <template slot-scope="scope">
                    ...
                    <span class="table-operate-btn" @click="handleEdit(scope.row,'edit')" v-show="isShowBtn('dynamicsInfo','资讯动态','Update')">编辑</span>
...
<script>
    ...
    import {retrieveInfodynamics,retrieveInfodynamicsDelete} from '@/api/api'
    export default {
        components:{
            Pagination,
            addModule
        ...
        methods:{
            ...
            handleEdit(row,flag) {
                ...
                if(flag=='delete') {
                    vm.$confirm('您确定要删除选中的内容？',{
                        ...
                        retrieveInfodynamicsDelete({id:row.id}).then(res=>{
                            if(res.code == 2000) {
            ...
            async getData(){
                this.loadingPage = true
                 retrieveInfodynamics(this.formInline).then(res => {
