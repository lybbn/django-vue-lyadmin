...
<!--                </el-form-item>-->
                ...
                <!--                    </el-date-picker>-->
                <!--                </el-form-item>-->
                <!--                v-show="isShowBtn('recyclerManage','回收员申请','Create')"-->
<!--                <el-form-item label=""><el-button size="small" @click="addModule" type="primary">新增</el-button></el-form-item>-->
        ...
        <div class="table">
            <el-table size="small" height="calc(100vh - 260px)" border :data="tableData" v-loading="loadingPage" style="width: 100%">
                ...
                <el-table-column min-width="90" prop="images3" label="用户头像">
                    <template slot-scope="scope">
                        <img  :src="scope.row.images3" style="width: 30px;height: 30px">
                    </template>
                </el-table-column>
                <el-table-column min-width="120" prop="name" label="回收员"></el-table-column>
                <el-table-column min-width="130" prop="contact" label="手机号"></el-table-column>
                <el-table-column min-width="160" prop="idcard" label="身份证件号"></el-table-column>
                <el-table-column min-width="150" prop="area" label="代理商位置"></el-table-column>
...
<!--                        </el-switch>-->
                ...
                <el-table-column label="操作" fixed="right" width="180">
                    <template slot-scope="scope">
                        <!--                        v-show="isShowBtn('recyclerManage','回收员申请','Update')"-->
                        <span class="table-operate-btn" @click="handleEdit(scope.row,'audit')" v-show="isShowBtn('recyclerApply','回收员申请','Audit')" v-if="scope.row.audit_status==10">审核</span>
                        <span class="table-operate-btn" @click="handleEdit(scope.row,'detail')" v-show="isShowBtn('recyclerApply','回收员申请','Retrieve')">详情</span>
                        <span class="table-operate-btn" @click="handleEdit(scope.row,'delete')" v-show="isShowBtn('recyclerApply','回收员申请','Delete')">删除</span>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
<script>
    ...
    import {retrieveRecycleraudit,retrieveRecyclerauditDelete} from '@/api/api'
    ...
    export default {
        ...
        methods:{
            ...
            handleEdit(row,flag) {
                ...
                if(flag=='delete') {
                    let vm = this
                    vm.$confirm('您确定要删除该回收员申请？',{
                        closeOnClickModal:false
                    }).then(res=>{
                        retrieveRecyclerauditDelete({id:row.id}).then(res=>{
                            if(res.code == 2000) {
            ...
            async getData(){
                this.loadingPage = true
                 retrieveRecycleraudit(this.formInline).then(res => {
