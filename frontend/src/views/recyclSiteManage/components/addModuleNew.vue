    ...
    <el-dialog
        ...
        <el-form  :inline="false" :model="formData" :rules="rules" ref="rulesForm" label-position="right" label-width="130px">
            ...
            <el-form-item label="站点位置：" prop="selectedOptions">
                <el-select  v-model="formData.sitearea" @change="changeOnsiterSelect1" placeholder="请选择区域"  style="width: 300px" :disabled="loadingTitle=='详情'">
                    <el-option
            ...
            <el-form-item label="头像：" prop="avatar" >
                <el-image :src="userinfo.avatar" style="width: 60px;height:60px"  :onerror="defaultImg"></el-image>
...
<script>
    import {retrieveRecyclingsiteAdd,retrieveRecyclingsiteEdit,getProvince,getCityDistrictByID,retrieveRecyclerusers,getAddressaccuracy,retrieveGetagentareaaddresslist,retrieveGetrecyclerusersbyarealist} from "@/api/api";
    export default {
        name: "addModule",
        data() {
            return {
        ...
        methods:{
            ...
            //根据选择的代理区域获取对应区域的站点员列表
            changeOnsiterSelect1(data){
                this.peopleList = []
                var objValue={}
                this.selectedOptions.forEach(function (item,index) {
                    if(item.id == data){
                        objValue = item
                    }
                })
                this.formData.province = objValue.province
                this.formData.city = objValue.city
                this.formData.district = objValue.district

                var params={
                    id: data,
                }
                 retrieveGetrecyclerusersbyarealist(params).then(res => {
                     if(res.code ==2000) {
            ...
            addModuleFn(item,flag) {
                ...
                if (item){
                    ...
                    this.formData.user = item.info.id
                    this.formData.sitearea  = item.info.areasid
                    this.peopleList = [{id:this.formData.user,avatar:item.info.avatar,mobile:item.info.mobile,name:item.info.name}]
                }else{
                   this.userinfo = {
                        avatar:'',
                        mobile:'',
                ...
                this.getretrieveGetagentareaaddresslist()
            },
            submitData() {
            ...
            getRecyclerUsers(){
             ...
             //根据代理区域获取回收员列表
            getretrieveGetagentareaaddresslist(){
                this.selectedOptions = []
                 retrieveGetagentareaaddresslist().then(res => {
                     if(res.code ==2000) {
