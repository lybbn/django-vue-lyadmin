    ...
    <div>
        <div class="tableSelect">
            <el-form :inline="true" :model="formInline" label-position="left">
                <el-form-item label="">
                    <el-button size="small" type="primary" @click="addModule" v-show="isShowBtn('recyclCategoryParent','一级分类','Create')">新增</el-button>
                </el-form-item>
                <el-form-item label="">
                    <el-button size="small" @click="handleDelete" type="danger" :disabled="multiple" v-show="isShowBtn('recyclCategoryParent','一级分类','Delete')">删除</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="table">
...
<!--                <el-table-column type="index" width="60" align="center" label="序号"></el-table-column>-->
                ...
                <el-table-column label="操作" fixed="right" width="180">
                    <template slot-scope="scope">
                        ...
                        <span class="table-operate-btn" @click="handleEdit(scope.row,'edit')" v-show="isShowBtn('recyclCategoryParent','一级分类','Update')">编辑</span>
                        <span class="table-operate-btn" @click="handleEdit(scope.row,'delete')" v-show="isShowBtn('recyclCategoryParent','一级分类','Delete')">删除</span>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
<script>
    ...
    import {retrieveRetrievecategory1,retrieveRetrievecategory1Delete} from '@/api/api'
    export default {
        components:{
            Pagination,
            addModule,
        },
        name:'recyclCategoryManage',
        data() {
            return {
                loadingPage:false,
                // 选项框选中数组
        ...
        methods:{
            ...
            handleEdit(row,flag) {
                ...
                if(flag=='delete') {
                    ...
                    vm.$confirm('确定删除该数据吗？',{
                        ...
                        retrieveRetrievecategory1Delete({id:row.id}).then(res=>{
                            if(res.code == 2000) {
                                vm.$message.success(res.msg)
                                vm.search()
                            } else {
            ...
            async getData(){
                this.loadingPage = true
                 retrieveRetrievecategory1(this.formInline).then(res => {
